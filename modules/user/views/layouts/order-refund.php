...
$urlHandle = $urlManager->createUrl(['user/mch/order/refund-handle']);
...
<!-- 退款 -->
<div class="modal fade" id="retreatmoneyModal" data-backdrop="static">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">提示</h5>
            ...
            <div class="modal-body">
                <input type="hidden" id="retreat_id1" name="retreat_id1" value="">

                <div class="mb-3">确认同意退款？同意后确认退款金额返还给客户。</div><div>请输入退款金额（最多<span id="max_price1"></span>元）：</div>
                <input type="number" id="retreat_price1" class="form-control" style="margin:10px 0" name="retreat_price1"
                       min="0.01" max="" step="0.01" value="">
...
<script>
    ...
    function refund_retreatmoney(id,retreat_price){
        $("#retreat_id1").val(id);
        $("#max_price1").text(retreat_price);
        $("#retreat_price1").attr("max",retreat_price);
        $("#retreat_price1").val(retreat_price);
    };
    ...
    //同意退款
    $(document).on("click", ".retreat-submit", function () {
        var id = $("#retreat_id1").val();
        var retreat_price = $("#retreat_price1").val();
        $("#retreatmoneyModal").modal("hide");
        $.myLoading({
            title: "正在提交"
        });
        $.ajax({
