...
<script>
    ...
    var initData = {
        ...
        model_group_list: isModels ? JSON.parse('<?= isset($models) ? json_encode($models->getAttrData()) : [] ?>', true) : [],
...
//        initData.use_attr = 0;
        ...
        // TODO 积分商城没有分销
    } else if(pageType === 'MODELS') {
        initData.model_group_count = JSON.parse('<?= isset($models) ? json_encode($models->getAttrData()) : [] ?>', true).length;
    }
