        ...
        <form class="auto-form" method="post" return="<?= $returnUrl ?>">
            ...
            <div class="step-block" flex="dir:left box:first" v-if="model">
                ...
                <div>
                    <div class="form-group row">
                        ...
                        <div class="col-9">
                            <input hidden type="text" v-model="model.id" name="goods_id">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-9">
                            <input class="form-control short-row" name="labels"
                                   v-model="model.labels">
                            <div class="fs-sm text-muted">例子：精品热卖,热销。多个请使用英文逗号<i
                                        style="display: inline-block;padding: 0.25rem;background: #e3e3e3;border-radius: .25rem;line-height: .5rem">,</i>分隔
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <div class="col-3 text-right">
                            <label class="col-form-label">是否加入今日特价</label>
                        </div>
                        <div class="col-9">
                            <label class="radio-label">
                                <input type="radio" name="is_special" value="0" v-model="model.is_special">
                                <span class="label-icon"></span>
                                <span class="label-text">不加入</span>
                            </label>
                            <label class="radio-label">
                                <input type="radio" name="is_special" value="1" v-model="model.is_special">
                                <span class="label-icon"></span>
                    ...
                    <div class="form-group row">
                        <div class="col-3 text-right">
                            <label class="col-form-label">是否加入推荐</label>
                        </div>
                        <div class="col-9">
                            <label class="radio-label">
                                <input type="radio" name="is_recommend" value="0" v-model="model.is_recommend">
                                <span class="label-icon"></span>
                                <span class="label-text">不加入</span>
                            </label>
                            <label class="radio-label">
                                <input type="radio" name="is_recommend" value="1" v-model="model.is_recommend">
                                <span class="label-icon"></span>
                                <span class="label-text">加入</span>
                            </label>
                        </div>
            ...
            <div class="step-block" flex="dir:left box:first">
                ...
                <div>
                    <div class="form-group row">
                        <div class="col-3 text-right">
                            <label class=" col-form-label required">模型设置</label>
                        </div>
                        <div class="col-9">
                            <label class="radio-label">
                                <input type="radio" name="is_attrbute_use" value="0"
                                       v-model="model.is_attrbute_use">
                                <span class="label-icon"></span>
                                <span class="label-text">不设置</span>
                            </label>
                            <label class="radio-label">
                                <input type="radio" name="is_attrbute_use" value="1"
                                       v-model="model.is_attrbute_use">
                                <span class="label-icon"></span>
                                <span class="label-text">设置</span>
                            </label>
                        </div>
                    </div>
                </div>
            ...
            <!--产品参数设置-->
            <div v-if="model.is_attrbute_use == 1">
                <div class="step-block" flex="dir:left box:first">
                    <div>
                        <span>产品模型</span>
                    </div>
                    <div>
                        <div class="form-group row">
                            ...
                            <div class="col-9">
                                <select class="form-control short-row addattr" name="attrbute_id" id="attrtype">
                                    <option value="">--</option>
                                    <template v-for="mo in attrmodel_list">
                                        <option v-if="mo.selected" selected :value="mo.id">{{mo.model_name}}</option>
                                        <option v-else :value="mo.id">{{mo.model_name}}</option>
                                    </template>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-9">
                                    <div v-for="(model_group,i) in model_group_list" :key="i" class="attr-group">
                                        <div>
                                            <input hidden type="text" v-bind:name="'model_group_name['+i+']'" v-bind:value="model_group.model_group_name">
                                            <b>{{model_group.model_group_name}}</b>
                                        </div>
                                        <div class="attr-group">
                                            <div v-for="(model,j) in model_group.model_list" class="attr-group">
                                                <div class="attr-item">
                                                    <input hidden type="text" v-bind:name="'model_name['+i+']['+j+'][model_name]'" v-bind:value="model.model_name">
                                                    <span class="attr-name">{{model.model_name}}</span>
                                                    <input class="form-control" type="text" v-bind:name="'model_name['+i+']['+j+'][model_name_value]'" v-bind:value="model.model_name_value">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res.code == 0) {
                ...
                app.attrmodel_list = res.data.attrmodel_list;
                ...
                app.model_group_list = res.data.model_group_list;
    ...
    $(document).on('click', '.mall-copy-btn', function () {
        ...
        $.myLoading();
        $.ajax({
            url: "<?=$urlManager->createUrl(['user/mch/goods/goods-copy'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                mall_id: mall_id,
            ...
            success: function (res) {
                ...
                btn.btnReset();
                if (res.code == 0) {
                    app.model.name = res.data.name;
                    app.model.virtual_sales = res.data.virtual_sales;
                    app.model.price = res.data.price;
                    app.model.unit = res.data.unit;
                    app.model.weight = res.data.weight;
                    app.model.service = res.data.service;
                    app.model.original_price = res.data.original_price;
                    // app.attr_group_list = res.data.attr_group_list;
                    // app.checked_attr_list = res.data.checked_attr_list;
                    app.model.mch_sort = res.data.mch_sort;
                    app.freight = res.data.freight;
                    app.attr_row_list = attrGroupGenerate(res.data.attr_group_list);
                    app.checked_attr_list = attrGroupGenerate(res.data.checked_attr_list);
                    ue = UE.getEditor('editor', {
                        serverUrl: "<?=$urlManager->createUrl(['upload/ue'])?>",
                        enableAutoSave: false,
                        saveInterval: 1000 * 3600,
                    ...
                    app.model.cover_pic = res.data.cover_pic;
...
//        app.attr_row_list = attrGroupGenerate(app.attr_group_list);
    ...
    $(function () {
        // console.log('执行')
        $(document).on('change', '#attrtype', function (item) {
            getModelsList();
        })
    })
    ...
    function getModelsList() {
        ...
        var models_id = $("#attrtype option:selected").val();
        console.log(goods_id);
        console.log(models_id);
        $.ajax({
            url: "<?=$urlManager->createUrl(['user/mch/goods/get-models-list'])?>",
            data: {
                goods_id: goods_id,
                models_id: models_id,
            },
