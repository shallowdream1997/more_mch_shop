                ...
                <tr>
                    ...
                    <td class="nowrap">
                        <?php if ($parameter['is_use'] == 1) : ?>
                            ...
                            <span>|</span>
                            <a class="goods-status-switch-0" data-id="<?= $parameter['id']?>"
                               href="javascript:">关闭</a>
                        <?php else : ?>
                            <span class="label-text">关闭</span>
                            <span>|</span>
                            <a class="goods-status-switch-1" data-id="<?= $parameter['id']?>"
                               href="javascript:">启动</a>
                        <?php endif ?>
                    </td>
                    <td class="nowrap">
                        <?= $parameter['sort'] ?>
...
<script>
    ...
    $(document).on('click', '.goods-status-switch-0', function () {
        ...
        $.loading('正在处理');
        $.ajax({
            url: '<?=Yii::$app->urlManager->createUrl(['user/mch/goods/attrbute-up-down'])?>',
            data: {
                id: id,
                type: 'down',
            },
    ...
    $(document).on('click', '.goods-status-switch-1', function () {
        var checkbox = $(this);
        var id = checkbox.attr('data-id');
        $.loading('正在处理');
        $.ajax({
