...
<div class="panel mb-3" id="app">
    ...
    <div class="panel-body">
                ...
                <table class="table table-bordered bg-white">
                    <tr>
                        <td colspan="5">
                            ...
                            <?php endif; ?>
                            <span class="mr-5"><span class="titleColor">订单号（微信交易）：</span><?= $order_item['order_no'] ?></span>
                            <span class="mr-5"><span class="titleColor">订单编号（一小时对接）：</span><?= $order_item['order_sn'] ?></span>
                            <span>
                            ...
                            <?php if ($order_item['refund']) : ?>
                                <div class="titleColor mr-1">售后状态：
                                    <?php if ($order_item['refund'] == 0) : ?>
                                        <span class="badge badge-success">待商家处理</span>
                                    ...
                                    <?php elseif ($order_item['refund'] == 3) : ?>
                                        <span class="badge badge-success">已拒绝退换货</span>
                                    <?php elseif ($order_item['refund'] == 4) : ?>
                                        <span class="badge badge-success">已同意退款</span>
                                    <?php endif; ?>
                                </div>
                            <?php endif; ?>
                        </td>
                    ...
                    <tr>
                        ...
                        <td class="order-tab-5">
                            <?php if (($order_item['is_pay'] == 1 || $order_item['pay_type'] == 2) && $order_item['is_confirm'] != 1 &&$order_item['is_new_sale'] != 1) : ?>
                            <div>
                                <a class="btn btn-sm btn-primary send-btn mb-2" href="javascript:"
                                   data-order-id="<?= $order_item['id'] ?>" data-express='<?= $order_item['express'] ?>'
                                   data-express-no='<?= $order_item['express_no'] ?>' data-words='<?= $order_item['words'] ?>'><?= ($order_item['is_send'] == 1) ? "修改快递单号" : "发货" ?></a>
                                <?php endif; ?>
                    ...
                    <?php if ($order_item['anhour_api_text']) : ?>
                        <tr>
                            <td colspan="5">
                                <div>
                                    <span class="mr-3"><span class="titleColor">一小时对接：</span><?= $order_item['anhour_api_text'] ?></span>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
        ...
        <div class="modal fade send-modal" data-backdrop="static">
            <div class="modal-dialog modal-sm" role="document" style="max-width: 850px">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form class="send-form" method="post">
                                ...
                                <div class="col-9">
                                    <div class="pt-1">
                                        <label class="custom-control custom-radio">
                                            <input id="radio0" value="2" checked
                                                   name="is_express" type="radio"
                                                   class="custom-control-input is-express">
                                            <span class="custom-control-indicator"></span>
                                            <span class="custom-control-description">一小时ERP系统发货</span>
                                        </label>
                                        <label class="custom-control custom-radio">
                                            <input id="radio1" value="1"
                                                   ...
                                                   class="custom-control-input is-express">
                                            <span class="custom-control-indicator"></span>
                                            <span class="custom-control-description">本系统快递发货</span>
                                        </label>
                                        <label class="custom-control custom-radio">
                            ...
                            <!--一小时ERP系统 -->
                            <div class="is-true-express-add">
                                <div class="modal-body">
                                    <template>
                                        <div class="form-group">
                                            <div class="input-group">
                                                <input class="form-control keyword" placeholder="请输入批次号查询">
                                                <span class="input-group-btn">
                                                    <div v-on:click.stop="showBatch()" class="btn btn-warning">
                                                        查找
                                                    </div>
                                                </span>
                                            </div>
                                        ...
                                        <div style="max-height:400px;overflow: auto">
                                            <table class="table table-bordered">
                                                ...
                                                <tr v-for="(item,index) in batch_list">
                                                    <td>{{item.depot_id}}</td>
                                                    <td>{{item.batch_number}}</td>
                                                    <td>{{item.depot_name}}</td>
                                                    <td>{{item.num}}</td>
                                                    <td>{{item.frozen_num}}</td>
                                                    <td>{{item.cost}}</td>
                                                    <td>
                                                        <input @click="selectRadio(item.depot_id,item.batch_number,item.depot_name)" id="batch-radio" name="batch-radio" type="radio" :value="item.depot_id">
                                                    </td>
                                                </tr>
                                            </table>
                                        </div>
                                    </template>
                                    <table class="table table-bordered" id="chukumessage" hidden>
                                        <label>出库信息</label>
                                    </table>
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tr v-for="(a,i) in addMList">
                                            <td>{{a.batch_number}}<input hidden type="text" v-model="a.batch_number"></td>
                                            <td>{{a.depot_name}}<input hidden type="text" v-model="a.depot_name"></td>
                                            <td><input v-model="a.num"></td>
                                            <td><input v-model="a.final_price"></td>
                                            <td><div v-bind:index="i" class="btn btn-sm btn-danger dellist">删除</div></td>
                                        </tr>
                                    </table>
                                </div>
                            ...
                            <div class="mt-2">
                                <label>商家留言（选填）</label>
                                <textarea class="form-control" name="words" id="words"></textarea>
                            </div>
                        </form>
                    </div>
...
<script>
    var app = new Vue({
        el: "#app",
        data: {
            depot_id:'', //随便定义的一个变量，记录你想要存的东西，他命名成啥可以,
            batch_no:'', //批次号
            batch_list: [],
            addList:[],
            addMList:[],
        ...
        methods: {
            ...
            selectRadio(e,e1,e2){
                ...
                this.unit = {};
                this.unit.depot_name = app.depot_name;
                this.unit.batch_number = app.batch_no;
                this.unit.depot_id = app.depot_id;
                this.addMList.push(this.unit);
                console.log(app.addMList)
            },
            deleteItem(e){
                console.log(e)
                if (e.target.innerText === "删除"){
                    this.unit = {};
                    this.addMList.splice(Number(e.target.id),1)
                    console.log(app.addMList)
                }
            },
            //批次号查询
            showBatch: function () {
                ...
                _self.batch_list = [];
                $.ajax({
                    url: '<?=$urlManager->createUrl(['user/mch/order/batch-number-import'])?>',
                    dataType: 'json',
                    type: 'get',
                    data: {
                        batch_number: keyword
                    },
    ...
    $(document).on("click", ".send-confirm-btn", function () {
        ...
        if (app.depot_id>0){
            var anhour = 1;
            var express = $("*[name='express_anhour']").val();
            var express_no = $("*[name='express_no_anhour']").val();
        }else {
            var anhour = 0;
            var express = $("*[name='express']").val();
            var express_no = $("*[name='express_no']").val();
        }
        // qu
        console.log(order_id);
        console.log(express);
        console.log(express_no);
        console.log(words);
        console.log(anhour);
        console.log(app.addMList);
        btn.btnLoading("正在提交");
        ...
        $.ajax({
            ...
            data: {
                _csrf:_csrf,
                depot_id: app.depot_id,
                batch_no: app.batch_no,
                order_id: order_id,
                express:express,
...
<script>
    ...
    $(document).on('click', '.is-express', function () {
        ...
        if (tr == 0) {
            $('.is-true-express').prop('hidden', true);
            $('.is-true-express-add').prop('hidden',true);
        } else if(tr == 1) {
            $('.is-true-express-add').prop('hidden',true);
            $('.is-true-express').prop('hidden', false);
        } else if(tr == 2) {
            $('.is-true-express').prop('hidden', true);
            $('.is-true-express-add').prop('hidden',false);
        }
    });
</script>
