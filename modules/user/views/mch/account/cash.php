    ...
    <div class="panel-body">
        <div class="p-3">
            <div flex="dir:left cross:center">
                <div style="font-size: 1.75rem;">本系统账户余额：</div>
        ...
        <div class="p-3">
            <div flex="dir:left cross:center">
                <div style="font-size: 1.75rem;">一小时系统账户余额：</div>
                <div style="color: #ff4544" class="mr-3" flex="dir:left cross:bottom">
                    <div style="font-size: 1.75rem"><?= $account_shop_money ?></div>
                    <div style="font-size: .75rem;line-height: 2.5">元</div>
                </div>
            </div>
        </div>
        <div class="p-3">
            <?php if ($cash_user): ?>
                <div style="font-size: 1.75rem;">首次绑定提现用户：</div>
                <div style="color: #ff4544" class="mr-3" flex="dir:left cross:bottom">
                    <div style="font-size: .75rem">姓名：<?= $cash_user->nickname ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;手机号码： <?= $cash_user->binding ?></div>
                </div>
            <?php else: ?>
                <div>
                    <a class="btn btn-primary" href="javascript:" data-toggle="modal" data-target="#edit"
                       data-backdrop="static">设置绑定微信提现用户</a>
                </div>
            <?php endif ?>
        </div>
    </div>
...
<div id="cashApply">
    ...
    <div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">设置微信默认提现用户</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <template v-if="show_user_list.length>0">
                                ...
                                <tr v-for="(item,index) in show_user_list">
                                    ...
                                    <td>
                                        <a class="btn btn-primary btn-sm send" href="javascript:"
                                           data-url="<?= $urlManager->createUrl(['user/mch/account/cashuser-edit', 'status' => 1]) ?>"
                                           :data-index="item.id">设为默认提现用户</a>
                                    </td>
                                </tr>
                            </table>
                        </div>
...
<script>
    ...
    var cashApply = new Vue({
        ...
        data: {
            ...
            show_user_list:<?=$user_list ? $user_list : '[]'?>
        },
        methods: {
            //关键字查询
            showKeyword: function () {
                ...
                $.ajax({
                    url: '<?=$urlManager->createUrl(['user/mch/account/get-user'])?>',
                    dataType: 'json',
                    type: 'get',
                    data: {
                        keyword: keyword
