...
<div class="row home-row" id="app" style="display: none">
    ...
    <?php if (!empty($mch)): ?>
        <div class="home-col col-md-12">
            <select id="pid" class="form-control panel_1" onchange="MchUpdate(this)">
                <?php foreach ($mch as $item) : ?>
                    <option value="<?= $item->id ?>"
                        <?= $item->is_default == 1 ? 'selected' : null ?>><?= $item->name ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    <?php endif; ?>
...
<script>
    function MchUpdate(e){
        console.log(e.options[e.selectedIndex].value);
        var mch_id = e.options[e.selectedIndex].value;
        var _csrf = "<?=Yii::$app->request->csrfToken?>";
        $.ajax({
            url: "<?=$urlManager->createUrl(['user/mch/index/change-mch']) ?>",
            type: 'post',
            data: {
                mch_id:mch_id,
                _csrf: _csrf
