...
class IndexController extends Controller
{
    ...
    public function actionIndex()
    {
        if (\Yii::$app->request->isAjax) {
            ...
            $mch = MchAuthLogin::find()->where(['user_id'=>\Yii::$app->user->id])->select('id,account_shop_id,account_id,is_default,name')->all();
    ...
    {
        $res = MchAuthLogin::updateAll(['is_default'=>0],['is_default'=>1,'store_id'=>$this->store->id,'binding'=>\Yii::$app->user->identity->binding,'user_id'=>\Yii::$app->user->identity->id]);
        if ($res){
            $result = MchAuthLogin::updateAll(['is_default'=>1],['store_id'=>$this->store->id,'id'=>\Yii::$app->request->post('mch_id')]);
            if ($result){
                return [
                    'code' => 0,
                    'msg' => '切换商户账号成功'
