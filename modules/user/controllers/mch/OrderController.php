...
use app\modules\user\models\mch\StoreOrderListForm;
...
class OrderController extends Controller
{
    ...
    {
        $form = new StoreOrderListForm();
        $form->store_id = $this->store->id;
        $form->mch_id = $this->mch->id;
        $form->limit = 10;
        $form->attributes = \Yii::$app->request->get();
        $data = $form->search();
        return $this->render('storeorder', [
            'row_count' => $data['row_count'],
            'pagination' => $data['pagination'],
            'list' => $data['list'],
            'express_list' => $this->getExpressList(),
    ...
    public function actionSend()
    {
        ...
        $form->store_id = $this->store->id;
        $form->mch_id = $this->mch->id;
        $form->account_shop_id = $this->mch->account_shop_id;
        return $form->save();
    }

    //批次号检测出一小时erp库存,供订单发货给商户选择库存id
    public function actionBatchNumberImport()
    {
        $form = new OrderSendForm();
        $form->attributes = \Yii::$app->request->get();
        $form->store_id = $this->store->id;
    ...
    //平台订单详情
    public function actionStoredetail($order_id = null)
    {
        $order = Order::find()->where(['store_id' => $this->store->id, 'id' => $order_id, 'mch_id' => 0])->asArray()->one();
        if (!$order) {
            $url = \Yii::$app->urlManager->createUrl(['user/mch/order/storeorder']);
            $this->redirect($url)->send();
        }
        $order['integral_arr'] = json_decode($order['integral'], true);
