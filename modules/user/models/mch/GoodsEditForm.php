...
class GoodsEditForm extends UserModel
{
    ...
    public $attrbute_id;
    public $model_group_name; //产品模型参数
    public $model_name;

    public $is_attrbute_use; //模型设置 0-否 1-是
    /**
     * @return array
     */
    public function rules()
    {
        return [
            ...
            [['cat_id', 'name', 'cover_pic','is_special','is_recommend'], 'required'],
            ...
            [['service','labels'], 'string', 'max' => 255],
            ['freight', 'integer', 'min' => 0],
            ['store_cat_id', 'integer',],
            ['use_attr', 'integer',],
            ['is_special', 'integer',],
            ['is_recommend', 'integer',],
            ['is_attrbute_use', 'integer',],
            ['goods_num', 'integer', 'min' => 0],
            ['attr', 'safe',],
            ['detail', 'string',],
            [['attrbute_id'],'default', 'value' => 0],
            ['model_group_name', 'safe',],
            ['model_name', 'safe',],
            ['detail', function () {
    ...
    public function attributeLabels()
    {
        return [
            ...
            'detail' => '图文详情',
            'attrbute_id' => '参数ID',
            'model_group_name' => '产品模型规格',
            'model_name' => '产品模型规格',
            'is_special' => '今日特价',
            'is_recommend' => '今日推荐',
            'is_attrbute_use' => '模型设置',
        ];
    }
    ...
    public function search()
    {
        ...
        $modelsList = AttrModel::find()->where(['store_id'=>$this->mch->store_id,'is_use' => 1])
            ->orderBy('sort DESC')
            ->select('id,model_name')->asArray()->all();
        foreach ($modelsList as &$mo){
            $mo['selected'] = ($this->model->attrbute_id && $this->model->attrbute_id == $mo['id']);
        }

//        $tplus_data = YongyouIsv::findOne(1);
//        $options = [
//            'appkey' => $tplus_data->isv_appkey, // ISV账号的AppKey <必须>
//            'appsecret' => $tplus_data->isv_appsecret, // ISV账号的AppSecret <必须>
//            'cert' => $tplus_data->cert, // 申请ISV账号审核通过后下发的pem版证书，使用cjet_pri.pem文件 <必须>
//            'orgid' => $tplus_data->orgid, // 企业云账号 <非账套模式必须，即authmode=ecloud>
//            'authmode' => $tplus_data->authmode, // 认证模式 account-账套 ecloud-企业云账号模式
//            'account' => [ // 账套账号配置 <account模式下必须>
//                'id' => $tplus_data->account_id, // 账套账号ID <account模式下必须>
//                'password' => $tplus_data->account_password, // 账套账号密码 <account模式下必须>
//                'number' => $tplus_data->account_number, // 账套编号 <account模式下必须>
//            ],
//        ];
//        $num_data = [
//            'Warehouse' => 'ssss',
...
//        $tplusApi::createAuthorizationHeader($authorizationHeader);
//        $tplusApi::setAPIUrl('TPlus/api/v2/currentStock/Query');
//        $tplusApi::setFields($num_data);
        return [
            'code' => 0,
            'data' => [
                'store_cat_list' => $this->getStoreCatList(),
                'cat_list' => $cat_list,
                'attrmodel_list' => $modelsList,
                'model_group_list' => json_decode($this->model->attrbute_json,true),
                ...
                'model' => [
                    ...
                    'is_recommend' => $this->model->is_recommend,
                    'is_attrbute_use' => $this->model->is_attrbute_use,
                ],
                'setting' => $setting
            ],
        ];
    ...
    public function save()
    {
        ...
        if ($this->use_attr) {
            if (!$this->attr || !is_array($this->attr) || !count($this->attr)) {
                ...
                $res = $this->getNo($this->attr);
                unset($res['']);
                if (!empty($res)){
                    return [
                        'code' => 1,
                        'msg' => '检测到有重复sku编号，请不要重复填写',
                    ];
                }
            }
            $minPrice = 0;
        ...
        $object = json_encode($attr_group_list,JSON_UNESCAPED_UNICODE);
        ...
        $this->model->attributes = $this->getAttributes([
            ...
            'original_price', 'service','labels', 'freight', 'full_cut', 'use_attr', 'detail',
            'individual_share','share_type','share_commission_first','share_commission_second','share_commission_third',
            'is_recommend','is_special','attrbute_id'
        ]);
        if (!$this->is_attrbute_use){
            $object = '';
            $this->model->attrbute_id = 0;
            $this->model->is_attrbute_use = 0;
        }else{
            if ($this->attrbute_id == 0 && is_null($this->model_name)){
                return [
                    'code' => 1,
                    'msg' => '规格模型参数不能为空',
                ];
        ...
        $this->model->attrbute_json = $object;
     ...
     * @param string $key
     * @return array
     * sku查重，看是否存在,不存在则为 [] 空数组， 存在则返回 重复 编号的数量
     */
    private function getNo($arr,$key = 'no')
    {
