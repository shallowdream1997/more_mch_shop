...
class CouponSendForm extends UserModel
{
    public $user_id_list;
    public $store_id;
    public $coupon_id;
    ...
    public function save()
    {
        ...
        $user_list = User::find()->select('id')->where(['id' => $this->user_id_list, 'store_id' => $this->store_id,'belong_mch'=>$this->mch_id])->all();
        $count = 0;
        foreach ($user_list as $u) {
            $res = Coupon::userAddCoupon($u->id, $this->coupon_id,0,3,$this->mch_id);
            if ($res) {
                $count++;
            }
