...
class MchEditForm extends MchModel
{
    ...
    public function rules()
    {
        $rules = [
            ...
            [['realname', 'tel', 'name', 'province_id', 'city_id', 'district_id', 'address', 'mch_common_cat_id', 'service_tel','score'], 'required'],
    ...
    public function save()
    {
        ...
        $this->model->score = $this->score ? $this->score : 5;
        if($this->user_id){
        ...
        if ($this->model->save()) {
            ...
            $mch_data = [
                'id' => $this->model->account_shop_id,
                'service_phone' => $this->service_tel,
            ];
            CurlHelper::post('storemall/store/updateStore',$mch_data);
            //end

            MchAuthLogin::deleteAll(['mch_id'=>$this->model->id]);
            $mch_auth = new MchAuthLogin();
            $mch_auth->store_id = $this->store->id;
            $mch_auth->user_id = $this->model->user_id;
            $mch_auth->mch_id = $this->model->id;
            $mch_auth->account_shop_id = $this->model->account_shop_id;
            $mch_auth->binding = $this->tel;
            $mch_auth->is_default = 1;
            $mch_auth->addtime = time();
            $mch_auth->name = $this->name ? $this->name : '';
