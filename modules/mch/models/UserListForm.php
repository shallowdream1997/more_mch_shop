...
class UserListForm extends MchModel
{
    ...
    public function search()
    {
        $query = User::find()->alias('u')->where([
            ...
            'u.is_delete' => 0,
        ])->leftJoin(Mch::tableName() . ' m', 'm.id=u.shop_id')
        ...
        if ($this->flag === Export::EXPORT) {
            ...
            $newQuery->select([
                'u.*', 'm.name shop_name', 'l.name l_name', 'card_count' => $cardQuery, 'order_count' => $orderQuery,
        ...
        foreach ($list as &$item){
            ...
            $item['m_name'] = $this->getMchName($item['belong_mch']);
        }
        return [
            'row_count' => $count,
    ...
    public function getMchName($id){
        if ($id == 0){
            $parent_name = '无归属门店';
        }else {
            $parent = Mch::findOne($id);
