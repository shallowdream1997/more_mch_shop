...
class GoodsForm extends MchModel
{
    ...
    public $is_recommend; //推荐
    public $is_special; //特价
    public $attrbute_id;
    public $model_group_name; //产品模型参数
    public $model_name;

    public $is_attrbute_use; //是否设置模型
    ...
    {
        return [
            [['name', 'service', 'unit', 'goods_no', 'labels'], 'trim'],
            [['store_id', 'name', 'price', 'cat_id', 'detail', 'goods_pic_list', 'cover_pic', 'attr_setting_type'], 'required'],
            [['store_id', 'share_type', 'quick_purchase', 'hot_cakes', 'is_level', 'is_negotiable','is_special','is_recommend','is_attrbute_use'], 'integer'],
            [['price', 'original_price'], 'number', 'min' => 0.01, 'max' => 99999999],
            [['detail', 'service', 'cover_pic', 'video_url', 'goods_no', 'labels'], 'string'],
            [['name', 'goods_no', 'unit'], 'string', 'max' => 255],
            [['sort'], 'default', 'value' => 1000],
            [['attr', 'individual_share', 'full_cut', 'integral', 'goods_card','model_group_name','attrbute_id','model_name'], 'safe',],
            [['share_commission_first', 'share_commission_second', 'share_commission_third', 'freight', 'rebate', 'virtual_sales', 'individual_share', 'goods_no', 'confine_count', 'plugin', 'cost_price', 'quick_purchase','attrbute_id'], 'default', 'value' => 0],
            [['share_commission_first', 'share_commission_second', 'share_commission_third', 'rebate', 'weight', 'cost_price'], 'number', 'min' => 0, 'max' => 99999999],
            [['goods_num', 'virtual_sales', 'sort', 'freight', 'confine_count','attrbute_id'], 'integer', 'min' => 0, 'max' => 99999999],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'model_name' => '产品模型规格',
            'is_recommend' => '是否推荐',
            'is_special' => '是否是今日特价栏目',
            'is_attrbute_use' => '是否设置规格参数',
        ];
    }
    ...
    public function save()
    {
        if ($this->validate()) {
            ...
            $goods->attrbute_id = $this->attrbute_id;
            $goods->attrbute_json = $object;
            ...
            if ($goods->save()) {
                //保存商品sku 编号
                if ($this->use_attr){
                    $setGoodsNo_res = $this->setGoodsNo($goods->id,$this->attr);
                    if (!$setGoodsNo_res){
                        $t->rollBack();
                        return '已检测有和其他产品编号重复的编号，请设置产品规格编号No不能重复';
                    }
                }
     ...
     * 保存商品sku No表
     */
    private function setGoodsNo($id,$attr)
    {
        if (isset($attr) && count($attr) > 0){
            GoodsNo::deleteAll(['goods_id' => $id]);
            foreach ($attr as $item) {
                if ($item['no']){
                    $is_exist = GoodsNo::find()->where(['no'=>$item['no']])->exists();
                    if ($is_exist){
                        return false;
                    }else{
                        if ($item['no']){
                            $dataNo[] = ['goods_id' => $id, 'no' => $item['no']];
                        }
                    }
                }
            }
