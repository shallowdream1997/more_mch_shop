...
class PrinterController extends Controller
{
    ...
    {
        $model = Printer::findOne(['id' => $id, 'store_id' => $this->store->id, 'is_delete' => 0,'mch_id'=>0]);
        if (!$model) {
            $model = new Printer();
        } else {
    ...
    {
        $list = Printer::findAll(['store_id'=>$this->store->id,'is_delete'=>0,'mch_id'=>0]);
        $model = PrinterSetting::findOne(['store_id'=>$this->store->id,'mch_id'=>0]);
        if (!$model) {
