...
class IndexController extends Controller
{
    ...
    //门店设为默认
    public function actionMchUpDown($id = null, $type = 1)
    {
        $mch = Mch::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id]);
        if (!$mch) {
            return [
                'code' => 1,
                'msg' => '门店不存在或已删除，请刷新重试',
        ...
        if ($type == 1) {
            ...
            $mch->is_store = 1;
            $mch_1 = Mch::findOne(['is_store' => 1, 'store_id' => $this->store->id]);
            if ($mch_1) {
                $mch_1->is_store = 0;
                $mch_1->save();
            }
