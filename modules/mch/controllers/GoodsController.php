...
class GoodsController extends Controller
{
    ...
    {
        if ($type == 'down') {
            $goods = Goods::findOne(['attrbute_id' => $id]);
            if ($goods) {
                return [
                    'code' => 1,
                    'msg' => '有商品正在使用该模型，无法关闭',
            ...
            $attr = AttrModel::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id]);
            $attr->is_use = 0;
        } elseif ($type == 'up') {
            $attr = AttrModel::findOne(['id' => $id, 'is_delete' => 0, 'is_use' => 0, 'store_id' => $this->store->id]);
    ...
    public function actionBatch()
    {
        ...
        } elseif ($get['type'] == 5) { //批量启动模型
            $res = AttrModel::updateAll(['is_use' => 1], $condition);
        } elseif ($get['type'] == 6) { //批量关闭模型
            $goods = Goods::findAll(['attrbute_id'=>$goods_id_group,'store_id' => $this->store->id]);
            if ($goods) {
                return [
                    'code' => 0,
                    'msg' => '已有商品在使用模型，无法关闭',
    ...
    public function actionGetModelsList($goods_id = 0,$models_id = 0)
    {
        ...
        if ($goods->attrbute_id == $models_id){
            ...
            $list = AttrModel::find()->select('model_group_json')
                ->where(['mch_id' => 0,'is_use' => 1,'store_id' => $this->store->id,'id' => $models_id])->one();
    ...
    public function actionParametersDel($id = 0)
    {
        ...
        if ($goods)
        {
            ...
            GoodsAttribute::updateAll(['is_delete' => 1],['id' => $id]);
            return [
                'code' =>0,
                'msg' => '删除成功',
            ];
    ...
    {
        $models = AttrModel::findOne(['id' => $id, 'store_id' => $this->store->id, 'mch_id' => 0]);
        if (!$models) {
