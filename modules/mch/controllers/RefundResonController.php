...
class RefundResonController extends Controller
{
    public function actionIndex()
    {
        $query = RefundReson::find()->where(['is_delete' => 0]);
        $count = $query->count();
    ...
    {
        $model = RefundReson::findOne(['id' => $id, 'is_delete' => 0]);
        if (!$model) {
            $model = new RefundReson();
        }
        if (\Yii::$app->request->isPost) {
            $form = new RefundResonForm();
            $form->model = $model;
