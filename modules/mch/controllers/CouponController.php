...
class CouponController extends Controller
{
    ...
    {
        $list = Coupon::find()->where(['store_id' => $this->store->id, 'is_delete' => 0,'mch_id'=>0])->orderBy('sort ASC')->all();
    ...
    public function actionEdit($id = null)
    {
        ...
        $goods = Goods::find()->where(['store_id' => $this->store->id, 'is_delete' => 0, 'status' => 1, 'id' => $goods_id,'mch_id'=>0])->all();
        if (!$model) {
            $model = new Coupon();
        }
...
//    查找商品
    public function actionSearchGoods($keyword)
    {
        ...
        $query = Goods::find()->alias('c')->where([
            'AND',
            ['LIKE', 'c.name', $keyword],
            ['store_id' => $this->store->id, 'is_delete' => 0, 'status' => 1, 'mch_id'=>0],
        ]);
