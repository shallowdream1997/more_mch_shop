...
class UserController extends Controller
{
     ...
     * 设置门店店员
     */
    public function actionParentClerkEdit($id = null, $status = 0,$clerk_id = 0)
    {
        ...
        if ($status == 1) {
            $clerk_exit = User::find()->where(['store_id' => $this->store->id, 'is_delete' => 0, 'id' => $clerk_id])->exists();
            if (!$clerk_exit) {
                return [
                    'code' => 1,
                    'msg' => '门店店员不存在',
            ...
            $user->shop_id = $clerk_id;
            $user->belong_mch = $clerk_id;
        }else{
            $user->shop_id = 0;
        }
        $t = \Yii::$app->db->beginTransaction();
        if ($user->save()) {
            $mem = MemberId::findOne(['user_id'=>$id,'is_delete'=>0]);
            if (!$mem){$mem = new MemberId();}
            $mem->user_id = $id;
            $mem->mch_id = $clerk_id;
            if ($status == 0){
                $mem->is_delete = 1;
    ...
    public function actionClerkEdit($id = null, $status = 0, $shop_id = 0, $edit = 0)
    {
        ...
        if ($status == 1) {
            $shop_exit = Mch::find()->where(['store_id' => $this->store->id, 'is_delete' => 0, 'id' => $shop_id])->exists();
            if (!$shop_exit) {
                return [
                    'code' => 1,
                    'msg' => '店铺不存在',
    ...
    public function actionClerk()
    {
        ...
        $shop_list = Mch::find()->where(['store_id' => $this->store->id, 'is_delete' => 0])->asArray()->all();
        $clerk_list = User::find()->where(['store_id' => $this->store->id, 'is_delete' => 0, 'is_clerk' => 1])->andWhere(['!=','belong_mch',0])->asArray()->all();
        return $this->render('clerk', [
            ...
            'clerk_list' => \Yii::$app->serializer->encode($clerk_list),
        ]);
    }
