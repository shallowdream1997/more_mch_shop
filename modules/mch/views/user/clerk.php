                ...
                <tr>
                    ...
                    <td>
                        <a class="btn btn-primary btn-sm user-shop-edit" data-id="<?= $u['id'] ?>"
                           href="javascript:">修改归属门店</a>
                        <?php if ($u['is_clerk'] == 1) : ?>
                            <a class="btn btn-sm btn-danger del" href="javascript:"
                               data-url="<?= $urlManager->createUrl(['mch/user/clerk-edit', 'id' => $u['id'], 'status' => 0]) ?>"
                               data-content="是否解除门店店员">解除门店店员</a>
                        <?php else : ?>
                            <a class="btn btn-info btn-sm user-clerk-edit" data-id="<?= $u['id'] ?>"
                               href="javascript:">修改归属店员</a>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
        ...
        <div id="app">
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">设置门店会员</h5>
                                ...
                                <div class="col-9">
                                    ...
                                    <template v-else>
                                        <label class="col-form-label">暂未设置门店，<a
                                                href="<?= $urlManager->createUrl(['mch/mch/index/index']) ?>">请前往设置</a></label>
                                    </template>
                                </div>
                            </div>
                            <template v-if="shop_list.length>0">
            ...
            <div class="modal fade" id="shop-edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">设置门店店员</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
            ...
            <!--修改归属店员-->
            <div class="modal fade" id="clerk-edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
                 aria-hidden="true" data-backdrop="static">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">设置归属店员</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <div class="form-group row">
                                <div class="col-3 text-right">
                                    <label class="col-form-label required">店员选择</label>
                                </div>
                                <div class="col-9">
                                    <template v-if="clerk_list.length>0">
                                        <div class="dropdown">
                                            <button class="btn btn-secondary dropdown-toggle" type="button"
                                                    id="dropdownMenuButton"
                                                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                {{select_clerk.nickname}}
                                            </button>
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton"
                                                 style="max-height: 200px;overflow-y: auto">
                                                <a class="dropdown-item clerk-select" href="javascript:"
                                                   v-for="(item,index) in clerk_list"
                                                   :data-index="index">{{item.nickname}}</a>
                                            </div>
                                        </div>
                                        <div class="clerk-error text-danger" hidden></div>
                                    </template>
                                    <template v-else>
                                        <label class="col-form-label">暂未设置门店店员，<a
                                                    href="<?= $urlManager->createUrl(['mch/user/clerk']) ?>">请前往设置</a></label>
                                    </template>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <a class="btn btn-primary editclerk" href="javascript:"
                               data-url="<?= $urlManager->createUrl(['mch/user/parent-clerk-edit', 'status' => 1]) ?>"
                               :data-index="edit_user_id">提交</a>
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">关闭</button>
                        </div>
                    </div>
...
//                }
    ...
    app.select_clerk = app.clerk_list.length > 0 ? app.clerk_list[0] : ""
</script>
<script>
    ...
    $(document).on('click', '.editclerk', function () {
        var a = $(this);
        var index = $(this).data('index');
        $('.clerk-error').prop('hidden', true);
        if (app.select_clerk == "") {
            $('.clerk-error').prop('hidden', false).html('请先选择门店店员');
            return;
        }
        $.ajax({
            url: a.data('url'),
    ...
    $(document).on('click', '.clerk-select', function () {
        var index = $(this).data('index');
        app.select_clerk = app.clerk_list[index];
    });
    $(document).on('click', '.user-clerk-edit', function () {
        if (app.user_list.length > 0) {
            app.edit_user_id = $(this).data('id');
            $('#clerk-edit').modal('show');
        }
    });
