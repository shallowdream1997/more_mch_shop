...
class OrderDetailForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        if ($order->is_pay == 0) {
            $status = '订单未付款';
            $store_data = Store::findOne(1);
            $timediff = $this->endtime($order->addtime,$store_data->over_day);
        ...
        if ($order->shop_id) {
            $shop = Mch::find()->select(['name', 'service_tel','tel', 'address', 'longitude', 'latitude'])->where(['store_id' => $this->store_id, 'id' => $order->shop_id])->asArray()->one();
        }
...
//                'goods_total_price' => doubleval(sprintf('%.2f', doubleval($order->total_price) - doubleval($order->express_price))),
                'goods_total_price' => $order->pay_price,
...
//        }
//
//        $start_time = date('Y-m-d H:i:s',$begin_time);
...
//    }
    ...
    //计算时间支付末期时间
    function endtime($begin_time,$hour)
    {
        $endtime = date("Y-m-d H:i:s",strtotime('+'.$hour.' hour',$begin_time));
        return $endtime;
    }
