...
class TeamForm extends ApiModel
{
    ...
    public function getNewOrder()
    {
        ...
        $mch = MchAuthLogin::find()->where(['user_id' => $this->user_id])->select('mch_id')->column();
//        $first = '(' . implode(',', $mch) . ')';

        $query = Order::find()->alias('o')
            ->leftJoin(['u' => User::tableName()],'u.id = o.user_id')
            ->where(['o.is_delete' => 0,'o.is_cancel' => 0])
        ...
        $list = $query->limit($pagination->limit)->offset($pagination->offset)->orderBy('o.addtime DESC')->select('o.*,u.nickname,u.avatar_url')->asArray()->all();

        $new_list = [];
        foreach ($list as $index => $value)
        {
            $new_list[$index]['order_type'] = "s"; //订单类型，s类型订单为普通商城订单
            $new_list[$index]['order_no'] = $value['order_no']; //订单编号
            $new_list[$index]['nickname'] = $value['nickname']; //订单下单用户昵称
            $new_list[$index]['avatar_url'] = $value['avatar_url']; //下单用户头像
            $new_list[$index]['is_price'] = $value['is_price']; //是否发放佣金
            $new_list[$index]['status'] = "待付款"; //待付款
            if ($value['is_pay'] == 0) {
                $new_list[$index]['status'] = "待付款";
            } elseif ($value['is_pay'] == 1 && $value['is_price'] == 0) {
                $new_list[$index]['status'] = "已付款";
            ...
            {
                //一级写入存在，匹配parent_id 里的商户id,如果一级id是我 有的门店id则
                if (in_array($value['parent_id'],$mch)){
                    $fir_name = "门店一级";
                    $fir_price = $value['first_price'];
                }else{
                    $fir_name = "";
                    $fir_price = 0;
                }
            ...
            if ($value['parent_id_1'] > 0)
            {
                //二级写入存在，如果二级里的id和我的用户id匹配，则这个订单的二级是我
                if ($value['parent_id_1'] == $this->user_id){
                    $sec_name = "员工二级";
                    $sec_price = $value['second_price'];
                }else{
                    $sec_name = "";
                    $sec_price = 0;
                }
            ...
            if ($fir_price > 0 && $sec_price > 0){ //如果两个都存在，则用户有两个佣金
                $new_list[$index]['share_status'] = $fir_name.' 和 '.$sec_name;
                $new_list[$index]['share_money'] = $fir_price + $sec_price;
            }else{
                if ($fir_name){ //如果有一个存在且一级存在，则用户为一级佣金
                    $new_list[$index]['share_status'] = $fir_name;
                    $new_list[$index]['share_money'] = $fir_price;
                }else{//如果有一个存在且二级存在，则用户为二级佣金
                    $new_list[$index]['share_status'] = $sec_name;
                    $new_list[$index]['share_money'] = $sec_price;
                }
            }
    ...
    public function getOrder()
    {
        ...
        $mch = MchAuthLogin::find()->where(['user_id' => $this->user_id])->select('mch_id')->column();
        $first = '(' . implode(',', $mch) . ')';
        $par_1 = \Yii::$app->user->identity->parent_id;
        ...
        if ($share_setting->level != 4) {
            ...
            if (!empty($mch)) {
                $where .= " OR (al.parent_id_2 = 0 AND al.user_id = {$par_1} AND al.parent_id_1 IN {$first}) ";
            }
    ...
    private function getOrderDetail($id)
    {
        $list = OrderDetail::find()->alias('od')->where(['od.order_id' => $id])
            ...
            ->select(['od.id', 'od.num', 'od.pic goods_pic', 'g.name', 'g.cover_pic','od.total_price'])
            ->asArray()->all();
        foreach ($list as $index => $value) {
            if (!$value['goods_pic']) {
    ...
    public function getTeamNew()
    {
        ...
        $UserShopId = User::findOne(['is_distributor' => 0,'shop_id' => 0]);
        if ($UserShopId){
            return [
                'code' => 0,
                'msg' => '',
...
//        $mch = MchAuthLogin::find()->where(['user_id' => $this->user_id])->select('mch_id')->column();
        ...
        $query = User::find()->where(['parent_id'=> $this->user_id]);
        $count = $query->count();
        $pagination = new Pagination(['totalCount' => $count, 'page' => $this->page - 1, 'pageSize' => $this->limit]);

        $list = $query->limit($pagination->limit)->offset($pagination->offset)->orderBy('time DESC')
            ->select('nickname,addtime,parent_id,id,avatar_url,price,time,shop_id')
            ->asArray()->all();

        foreach ($list as $index => $value) {
            $list[$index]['time'] = date('Y-m-d', $value['time']);
            $child_count = User::find()->where(['parent_id' => $value['id'], 'is_delete' => 0])->count();

            $total = Order::find()->alias('o')
                ->leftJoin(['odr'=>OrderRefund::find()
                    ->where(['type'=>1,'status'=>1])
                    ->select(['SUM(refund_price) as refund_price','order_id'])
                    ->groupBy('order_id')],'odr.order_id = o.id')
                ->where(['o.is_pay'=>1,'o.is_delete'=>0,'o.is_cancel'=>0])
                ->where(['o.user_id'=>$value['id']])
                ->select(['SUM(o.total_price)-ifnull(SUM(odr.refund_price),0) AS total_price,COUNT(o.id) AS totalcount'])->asArray()->one();

            $list[$index]['child_count'] = $child_count ? $child_count : 0; //下级总人数
            $list[$index]['order_price'] = $total['total_price'] ? $total['total_price'] : 0; //消费价格
            $list[$index]['order_count'] = $total['totalcount'] ? $total['totalcount'] : 0; //订单总数
            if ($value['shop_id']){
                $list[$index]['share_level'] = 1;
                $list[$index]['share'] = "一级门店店员";
            }else{
                $list[$index]['share_level'] = 2;
                $list[$index]['share'] = "二级普通用户";
            }
        }
        $data['list'] = $list;
        return [
