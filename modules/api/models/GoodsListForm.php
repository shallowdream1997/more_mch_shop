...
class GoodsListForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        if ($this->cat_id) {
            $cat = Cat::find()->select('id,name,advert_pic,advert_url')->where(['is_delete' => 0,])->andWhere(['OR', ['parent_id' => $this->cat_id], ['id' => $this->cat_id],])->one();
            ...
            $brand = ['cat_name'=>$cat->name,'advert_pic'=>$cat->advert_pic,'advert_url'=>$cat->advert_url];
        }
        ...
        $list = $query
            ...
            ->select('g.id,g.name,g.price,g.original_price,g.cover_pic pic_url,gn.num,g.virtual_sales,g.unit,g.is_negotiable,g.service,g.labels')
            ->limit($pagination->limit)
            ->offset($pagination->offset)
            ->asArray()->groupBy('g.id')->all();

        $catList = Cat::find()->where(['parent_id'=>0,'is_delete'=>0,'is_show'=>1,'is_search'=>1])
            ->orderBy('sort,addtime DESC')->select('id,name')->asArray()->one();
        $sub_list = Cat::find()->where([
            'parent_id' => $catList['id'],
            'is_delete' => 0,
            'is_show' => 1,
        ])->orderBy('sort,addtime DESC')->select('id,name')->asArray()->all();
        $catList['list'] = $sub_list;
        ...
        foreach ($list as $i => $item) {
            ...
            if (!$item['service']) {
                ...
                foreach ($option as $im) {
                    if ($im['is_default'] == 1) {
                        $service_list = explode(',', $im['service']);
                        break;
                    }
                }
            }
            ...
            if (is_array($service_list)) {
                foreach ($service_list as $im1) {
                    $im1 = trim($im1);
                    if ($im1) {
                        $new_service_list[] = $im1;
                    }
                }
            }
            $list[$i]['service'] = $new_service_list;
        }
        $data = [
            'row_count' => $count,
