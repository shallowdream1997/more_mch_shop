...
class OrderListForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        $list = $query->offset($pagination->offset)->limit($pagination->limit)->orderBy('o.addtime DESC')
            ->select('o.id,o.order_no,o.total_price,o.addtime,o.is_pay,o.is_send,o.is_confirm,o.is_delete,o.is_cancel,o.is_offline')
            ->asArray()->all();
    ...
    public function searchRefund()
    {
        ...
        $list = $query->offset($pagination->offset)->limit($pagination->limit)->orderBy('or.addtime DESC')
            ->select([
                ...
                'or.id', 'or.order_refund_no', 'or.refund_price', 'or.addtime order_refund_time', 'or.desc', 'or.type', 'or.pic_list', 'or.status', 'or.refuse_desc','or.is_agree','or.is_user_send'
            ])
            ->asArray()->all();
        ...
        $refund_address = RefundAddress::find()->where(['is_delete'=>0,'store_id'=>$this->store->id])->select('id,name,address,mobile')->asArray()->all();
