...
class TongjiMonthDataForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        return [
            ...
            'data' => [
                ...
                'up_rate' => $up_rate,
                'list' => $this->getMonthDataList($this->year,$this->month),
            ],
        ];
    }
    ...
    //月每日数据收成
    public function getMonthDataList($year,$month)
    {
        $start_time = strtotime("{$year}-{$month}-01");
        $end_time = strtotime("{$year}-{$month}-01 +1 month -1 second");
        $month_days = date("t", strtotime("{$year}-{$month}"));
        ...
        $list = $query->offset($pagination->offset)->limit($pagination->limit)->orderBy('addtime DESC')
            ->select('id,addtime,pay_price')
            ->asArray()->all();
