...
class AccountLogForm extends ApiModel
{
    ...
    {
        return [
            [['type', 'year', 'month', 'page', 'settle_type','status'], 'integer'],
        ];
    }
    ...
    public function search()
    {
        ...
        if ($this->year && $this->month){
            $start_time = strtotime("{$this->year}-{$this->month}-01");
            $end_time = strtotime("{$this->year}-{$this->month}-01 +1 month -1 second");
            $query->andWhere([
                'AND',
                ['>=', 'addtime', $start_time],
                ['<=', 'addtime', $end_time],
        ...
        foreach ($list as &$item) {
            $item['time'] = date('m月d日', $item['addtime']);
...
//                $list[$item['oid']]['name'] .= ',' . $item['name'];
    ...
    public function noSettleLog()
    {
        ...
        return [
            ...
            'msg' => '获取成功',
            'data' => [
                'nosettle_bg' => \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/shop/img/mch-account-header-bg-1.png',
                'settle_bg' => \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/shop/img/mch-account-header-bg-2.png',
                'no_settle_price' => $no_settle_price,
                'settle_price' => $settle_price
            ]
        ];
