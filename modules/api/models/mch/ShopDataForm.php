...
//    }
    ...
    public function search(){
        ...
        $list = $query->select('id,parent_id,name,icon')->asArray()->all();
        foreach ($list as $i => $item) {
            $sub_list = MchCat::find()->where([
                ...
                ->select('id,parent_id,name,icon')->asArray()->all();
    ...
    {
        $cat_list = MchCat::find()->where(['mch_id' => $this->mch_id, 'is_delete' => 0, 'parent_id' => 0])->orderBy('sort,addtime DESC')->asArray()->all();
    ...
    public function getMchGoodsList()
    {
        $query = Goods::find()->alias('g')->where([
            'g.is_delete' => 0,
            'g.mch_id' => $this->mch_id,
