                ...
                switch ($item['type']) {
                    ...
                    case 'goods':
                        ...
                        foreach ($param['list'] as $key => &$value) {
                            ...
                            if (count($value['goods_list']) > 0) {
                                foreach ($value['goods_list'] as $i => $k){
                                    $result = Goods::findOne($k['goods_id']);
                                    $service_list = explode(',', $result->service);
                                    // 默认商品服务
                                    if (!$result->service) {
                                    ...
                                    if (is_array($service_list)) {
                                        foreach ($service_list as $item2) {
                                            $item2 = trim($item2);
                                            if ($item2) {
                                                $new_service_list[] = $item2;
                                            }
                                        }
                                    }

                                    $value['goods_list'][$i]['service_list'] = $new_service_list;
                                }
                                $list[] = $value;
                            }
