...
class ShareQrcodeForm extends ApiModel
{
    ...
    public function goods_qrcode()
    {
        ...
        $pic_url = str_replace('http://', $http, \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/temp/' . $goods_pic_save_name);
...
//        $editor->blend($goods_qrcode, $wxapp_qrcode, 'normal', 1.0, 'top-left', 470, 1040);
    ...
    public function share_qrcode()
    {
        ...
        $pic_url = str_replace('http://', $http, \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/temp/' . $save_name);
        ...
        if (!$this->user->shop_id){
            return [
                'code' => 1,
                'msg' => '您尚未成为某门店店员，请先成为店员'
            ];
        }
...
//        $pic_url = \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/shop/img/test-shareqrcode.png';
//        $pic_url = str_replace('http://', $http, $pic_url);
        return [
            'code' => 0,
            'msg' => 'success',
            'data' => $pic_url . '?v=' . time()
