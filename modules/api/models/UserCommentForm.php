...
class UserCommentForm extends ApiModel
{
    public $store_id;
    public $user_id;
    public $score;
    ...
    public function search()
    {
        ...
        $query = OrderComment::find()->alias('oc')->leftJoin(['u' => User::tableName()], 'oc.user_id=u.id')
            ->where(['oc.is_delete' => 0, 'oc.is_hide' => 0,'oc.user_id' => $this->user_id]);
        $count = $query->count();
    ...
    public function countData()
    {
        ...
        $score_all = OrderComment::find()->alias('oc')
            ->where([ 'oc.is_delete' => 0, 'oc.is_hide' => 0,'oc.user_id' => $this->user_id])->count();
        $score_3 = OrderComment::find()->alias('oc')
            ->where([ 'oc.is_delete' => 0, 'oc.is_hide' => 0, 'oc.score' => 3,'oc.user_id' => $this->user_id])->count();
        $score_2 = OrderComment::find()->alias('oc')
            ->where([ 'oc.is_delete' => 0, 'oc.is_hide' => 0, 'oc.score' => 2,'oc.user_id' => $this->user_id])->count();
        $score_1 = OrderComment::find()->alias('oc')
            ->where([ 'oc.is_delete' => 0, 'oc.is_hide' => 0, 'oc.score' => 1,'oc.user_id' => $this->user_id])->count();
        return (object)[
