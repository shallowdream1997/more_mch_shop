...
class OrderRefundForm extends ApiModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['type',], 'in', 'range' => [1, 2,3]],
            [['pic_list','refund_reason','refund_status',], 'string'],
        ];
    }
    ...
    public function save()
    {
        ...
        if ($refund->type == 1) {
            $refund->refund_status = $this->refund_status ? $this->refund_status : '已收到货';// 货物状态
            ...
            $refund->refund_price = min($order->pay_price, $order_detail->total_price);
            $refund->refund_status = $this->refund_status ? $this->refund_status : '未收到货';// 货物状态
        }else {
            return [
                'code' => 1,
                'msg' => '未知售后类型'
        ...
        $refund->status = 0;
        $refund->addtime = time();
        $refund->refund_reson = $this->refund_reason ? $this->refund_reason : '无'; // 退款原因
//        $refund->refund_status = $this->refund_status ? $this->refund_status : ''; // 货物状态
        if ($refund->save()) {
            //售后订单申请成功之后，相关操作
            $form = new OrderWarn();
