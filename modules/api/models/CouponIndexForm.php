...
class CouponIndexForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        $list = $query->orderBy('uc.addtime DESC')
            ->limit(200)
            ->select('uc.id user_coupon_id,c.sub_price,c.min_price,uc.begin_time,uc.end_time,uc.is_use,uc.is_expire,cas.event,uc.type,c.appoint_type,c.cat_id_list,c.goods_id_list,c.rule,c.discount_type,c.discount')->asArray()->all();
        ...
        foreach ($list as $i => $item) {
            ...
            $list[$i]['sub_price'] = floatval($list[$i]['sub_price']);
            $list[$i]['min_price'] = floatval($list[$i]['min_price']);
        }
        return [
            'code' => 0,
            'msg' => 'success',
    ...
    public function indexcoupon()
    {
        ...
        $list = $query->orderBy('uc.addtime DESC')
            ->select('uc.id user_coupon_id,c.sub_price,c.min_price,uc.begin_time,uc.end_time,uc.is_use,uc.is_expire,cas.event,uc.type,c.appoint_type,c.cat_id_list,c.goods_id_list')->asArray()->one();
        ...
        $list['min_price_desc'] = $list['min_price'] == 0 ? '无门槛' : '满' . $list['min_price'] . '元可用';
        $list['begin_time'] = date('Y.m.d H:i', $list['begin_time']);
        $list['end_time'] = date('Y.m.d H:i', $list['end_time']);
        if (!$list['event']) {
            if ($list['type'] == 2) {
                $list['event'] = $list['event'] = 3;
            } elseif ($list['type'] == 0) {
                $list['event'] = $list['event'] = 0;
            } else {
                $list['event'] = $list['event'] = 4;
            }
        }
        $list['event_desc'] = $events[$list['event']];

        if ($list['appoint_type'] == 1) {
            $list['cat'] = Cat::find()->select('name')->where(['store_id'=>$this->store_id,'is_delete'=>0,'id'=>json_decode($list['cat_id_list'])])->asArray()->all();
            $list['goods'] = [];
        } elseif ($list['appoint_type'] == 2) {
            $list['goods'] = json_decode($list['goods_id_list']);
            $list['cat'] = [];
        } else {
            $list['goods'] = [];
            $list['cat'] = [];
        }
        $list['sub_price'] = floatval($list['sub_price']);
        $list['min_price'] = floatval($list['min_price']);
