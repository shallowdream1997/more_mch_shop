...
class GoodsForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        if ($this->user_id) {
            ...
            $coupon = null;
            //判断商品是否是门店商品，如果是则读取门店优惠券
            if ($goods->mch_id){
                $mch_coupon = $this->getMchCoupon($goods,$this->user_id);
            }else{
                $mch_coupon = $this->getCoupon($this->user_id);
            }
        }
        if ($goods->labels){
            $goods_labels = explode(',',$goods->labels);
        }else{
            $goods_labels = [];
        }
...
//        $icon = [
//            'home_icon' => \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/images/good-detail/home.png',
//            'cart_icon' => \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/images/good-detail/cart.png',
//            'share_icon' => \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/images/good-detail/share.png',
//            'user_icon' => \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/images/good-detail/user.png',
//            'three_bars_icon' => \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/images/good-detail/three-bars.png',
//        ];
        $data = [
            'id' => $goods->id,
            'pic_list' => $pic_list,
            ...
            'models_group_list' => json_decode($goods->attrbute_json),
            'num' => $goods->getNum(),
            'is_favorite' => $is_favorite,
            'service_list' => $new_service_list,
            'original_price' => sprintf('%.2f', $goods->original_price),
            'video_url' => $goods->video_url ? $goods->video_url : '',
            'unit' => $goods->unit,
...
//            'icon' => $icon,
    ...
    public function getShopList($mch_id = 0){
        $query = Mch::findOne($mch_id);
     ...
     * 门店优惠券
     */
    public function getMchCoupon($goods,$user_id){
        $c = new \app\modules\api\models\mch\CouponListForm();
        $c->mch_id = $goods->mch_id;
        $c->user_id = $user_id;
        $coupon = $c->getList();
        return $coupon;
    }
