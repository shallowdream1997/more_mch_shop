...
class OrderRefundDetailForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        $order_refund['time'] = $order_refund['addtime'] ? date('Y-m-d H:i:s',$order_refund['addtime']) : '未知';
        return [
            'code' => 0,
            'msg' => 'success',
            'data' => $order_refund,
    ...
    //撤销
    public function UndoRefundOrder()
    {
        if (!$this->validate()) {
            return $this->errorResponse;
        }
        $undo = OrderRefund::findOne(['id'=>$this->order_refund_id]);
        if ($undo->delete()){
            return [
                'code' => 0,
                'msg' => 'success',
