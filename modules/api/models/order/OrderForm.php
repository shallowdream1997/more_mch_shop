...
//            }
    ...
    protected function getCouponList(&$mch)
    {
        ...
        $list = UserCoupon::find()->alias('uc')
            ...
            ->select('uc.id user_coupon_id,c.sub_price,c.min_price,cas.event,uc.begin_time,uc.end_time,uc.type,c.appoint_type,c.cat_id_list,c.goods_id_list,c.discount_type,c.discount')
            ->asArray()->all();
        $events = [
            0 => '平台发放',
            1 => '分享红包',
     ...
     * Mch门店优惠券获取
     */
    protected function getMchCouponList(&$mch)
    {
        $goods_total_price = $mch['total_price'];
        $cat_ids = $this->getMchCatIdList($mch['goods_list']);
        $coupon_goods_id = $this->getGoodsIdList($mch['goods_list']);
     ...
     * Mch Goods Cat 分类获取
     */
    protected function getMchCatIdList(&$goods_list)
    {
        $cat_id_list = [];
        foreach ($goods_list as &$goods) {
            ...
            $cat_parent_list = MchCat::find()->select('parent_id')
                ->andWhere(['id' => $goods['cat_id'], 'is_delete' => 0])->andWhere(['>', 'parent_id', 0])
                ->column();
...
//                $forehead = (int)$forehead;
    ...
    {
        $query = Mch::find()->select(['address', 'service_tel as mobile', 'id', 'name', 'longitude', 'latitude'])
            ->where(['store_id' => $this->store_id, 'is_delete' => 0]);
//        if ($this->mch_list[0]['mch_id'] == 0){
//            $query->andWhere(['is_store'=>1]);
//        }else{
//            $query->andWhere(['id'=>$this->mch_list[0]['mch_id']]);
//        }
