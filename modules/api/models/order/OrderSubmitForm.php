...
use app\models\task\order\OrderIsv;
use app\models\User;
...
class OrderSubmitForm extends OrderForm
{
    ...
    public function save()
    {
        ...
        foreach ($mchListData as &$mch) {
            ...
            $order->order_sn = $this->getOrderSn(); //订单编号
            $order->order_no = $this->getOrderNo(); //订单号（微信交易流水号）
            ...
            if (isset($mch['picker_coupon']) && !empty($mch['picker_coupon'])) {
                ...
                $order->coupon_sub_price = $mch['picker_coupon']['sub_price'] ? $mch['picker_coupon']['sub_price'] : bcsub($mch['total_price'] + $mch['express_price'],$payPrice,2);
                $order->discount = $mch['picker_coupon']['discount'];
                // 查找优惠券优惠的商品
    ...
    private function getPayPrice($mch)
    {
        ...
        if (isset($mch['picker_coupon'])) {
            ...
            if ($pickerCoupon['sub_price'] >= 0) {
                ...
                if ($pickerCoupon['discount_type'] == 2) {
                    //满减
                ...
                } elseif ($pickerCoupon['discount_type'] == 1) {
                    //折扣
                    if ($coupon_price > 0) {
                        $mch['picker_coupon']['sub_price'] = ($coupon_price * (10 - $pickerCoupon['discount']))/10;
                        $payPrice = $payPrice - ($coupon_price * (10 - $pickerCoupon['discount']))/10;
                    } else {
                        $mch['picker_coupon']['sub_price'] = ($payPrice * (10 - $pickerCoupon['discount']))/10;
                        $payPrice = ($payPrice * $pickerCoupon['discount'])/10;
                    }
                    //折扣券计算 end
                }
            }
...
//                }
    ...
    public function getOrderSn()
    {
        ...
        while (true) {
            ...
            $exist_order_no = Order::find()->where(['order_sn' => $order_no])->exists();
            if (!$exist_order_no) {
                break;
            }
        }
