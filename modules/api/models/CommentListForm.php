...
class CommentListForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        if ($this->status==1){
            $query->andWhere(['!=','oc.pic_list','[]']);
        }elseif($this->status==2){
            $query->andWhere(['oc.score'=>3]);
        }elseif ($this->status==3){
            $query->andWhere(['oc.score'=>1]);
        }
        $count = $query->count();
        $pagination = new Pagination(['totalCount' => $count, 'pageSize' => $this->limit, 'page' => $this->page - 1]);
        $list = $query->limit($pagination->limit)->offset($pagination->offset)->orderBy('oc.addtime DESC')->asArray()
            ->select('oc.is_virtual,oc.virtual_user,oc.virtual_avatar,u.nickname,u.avatar_url,u.binding,oc.score,oc.content,oc.pic_list,oc.addtime,oc.reply_content')->all();
        ...
        $data = [
            ...
            'comment_icon' => \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/statics/wxapp/images/comment-icon.png',
            'list' => $list,
    ...
    public function countData()
    {
        ...
        $score_3 = OrderComment::find()->alias('oc')
            ->where(['oc.goods_id' => $this->goods_id, 'oc.is_delete' => 0, 'oc.is_hide' => 0, 'oc.score' => 3])->count(); //好评
        $score_2 = OrderComment::find()->alias('oc')
            ->where(['oc.goods_id' => $this->goods_id, 'oc.is_delete' => 0, 'oc.is_hide' => 0])->andWhere(['!=','oc.pic_list','[]'])->count(); //有图
        $score_1 = OrderComment::find()->alias('oc')
            ->where(['oc.goods_id' => $this->goods_id, 'oc.is_delete' => 0, 'oc.is_hide' => 0, 'oc.score' => 1])->count(); //差评
        if ($score_3){
            $positive_rate = round(bcmul(bcdiv($score_3,$score_all,2),100))."%";
        }else{
            $positive_rate = "0.00%";
        }
        return (object)[
