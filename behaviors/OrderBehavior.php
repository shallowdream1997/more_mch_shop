...
//            $order->share_price += doubleval($order->first_price);
            $mch_1 = Mch::findOne($order->parent_id);
            if (!$mch_1){
                $order->save();
                return;
            }
            $mch_1->account_money += $order->first_price;
            $mch_1->save();
            $order->is_price = 1;

            $log = new MchAccountLog();
            $log->store_id = $order->store_id;
            $log->mch_id = $order->parent_id;
            $log->type = 1;
            $log->price = $order->first_price;
            $log->desc = '订单（' . $order->order_no . '） 的一级门店佣金';
            $log->addtime = time();
            if (!$log->save()) {
