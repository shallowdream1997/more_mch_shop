...
class PinterOrder
{
    ...
    public $order_mch;//订单门店ID
    public $extra;


    public function __construct($store_id, $order_id, $print_type, $order_type = 0, $order_mch = 0)
    {
        ...
        $this->order_mch = $order_mch;
    }

    public function print_order()
    {
        try {
            $this->setting = PrinterSetting::findOne(['store_id' => $this->store_id, 'mch_id'=>$this->order_mch]);//打印设置
            if (!$this->setting) {
                \Yii::warning('未设置打印设置');
