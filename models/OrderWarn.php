...
class OrderWarn extends Model
{
    ...
    public function notify()
    {
        ...
        try {
            ...
            if ($is_print == 1) {
                ...
                $printer_order = new PinterOrder($order->store_id, $order->id, 'pay', $this->order_type,$this->order->mch_id);
    ...
    private function OrderNotify()
    {
        ...
        //首次下单用户绑定门店
        if (!\Yii::$app->user->identity->belong_mch){
            $this->UserBindMch($order->user_id,$order->mch_id);
        }
        //消费满指定金额自动成为分销商
     ...
     * 写入销售统计表
     */
    private function addMchTongji($order)
    {
        $mch_id = $order->mch_id;
        $y = date("Y",$order->pay_time);
        $m = date("m",$order->pay_time);
        $MchUpdate = MchOrderTongji::updateAllCounters(['month_order_sum'=>$order->pay_price],['mch_id'=>$mch_id,'year'=>$y,'month'=>$m]);
//        $Mch = MchOrderTongji::findOne(['mch_id'=>$mch_id,'year'=>$y,'month'=>$m]);

        if (!$MchUpdate){
            $Mch = new MchOrderTongji();
            $Mch->mch_id = $mch_id;
            $Mch->year = $y;
            $Mch->month = $m;
     ...
     * 用户首次下单绑定门店
     */
    public function UserBindMch($user_id,$mch_id)
    {
        User::updateAll(['belong_mch'=>$mch_id],['id'=>$user_id]);
    }
