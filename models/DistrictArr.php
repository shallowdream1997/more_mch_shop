...
class DistrictArr
{
    ...
    public static function getProCityDisId($select_name,$name)
    {
        $arr = self::getArr();
        // 提取出省,市,区 份数组
        $province = self::filter_by_value($arr, 'level', $select_name);
        // 转成一维数组，键名为id，键值为name
        $province_names = array_column($province, 'name', 'id');
        // 根据value查找对应的省份id
        $one = self::match_by_value($province_names, $name);
    ...
    */
     ...
     * 精确查找根据名称返回id
     */
    public static function match_by_value($array, $value){
